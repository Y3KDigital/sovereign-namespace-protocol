...
pub fn issue(
    ...
    // Load identity
    let identity: Identity = load_json(identity_file).context("Failed to load identity")?;

    // Load namespace
    let namespace: Namespace = load_json(namespace_file).context("Failed to load namespace")?;

    // Load secret key
    let seckey: DilithiumSecretKey = load_json(seckey_file).context("Failed to load secret key")?;

    // Parse claims root
    let claims_root = parse_hex_32(claims_hex).context("Invalid claims root hash")?;
    ...
    // Save to file
    save_json(output, &cert).context("Failed to save certificate")?;

    // Display results
    println!("{}", "✅ Certificate issued successfully!".green().bold());
    println!(
        "  Subject: {}",
        format!("0x{}", hex::encode(cert.subject)).bright_blue()
    );
    println!(
        "  Namespace: {}",
        format!("0x{}", hex::encode(cert.namespace)).bright_black()
    );
    ...
    println!(
        "  Content hash: {}",
        format!("0x{}", hex::encode(cert.content_hash())).bright_cyan()
    );
...
pub fn verify(file: &str, identity_file: &str, current_time: u64) -> Result<()> {
    ...
    // Load certificate
    let cert: Certificate = load_json(file).context("Failed to load certificate")?;
