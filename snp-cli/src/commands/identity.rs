...
pub fn create(namespace_file: &str, subject: &str, pubkey_file: &str, output: &str) -> Result<()> {
    ...
    let identity = Identity::derive(&namespace, subject, pubkey)
    ...
    println!("  Namespace: {}", format!("0x{}", hex::encode(identity.namespace_id)).bright_black());
...
pub fn verify(file: &str, namespace_file: &str) -> Result<()> {
    ...
    // Load identity
    let identity: Identity = load_json(file)
