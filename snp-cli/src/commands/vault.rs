use crate::utils::{load_json, parse_hex_32, save_json};
...
pub fn derive(
    ...
    // Parse hashes
    let asset_hash = parse_hex_32(asset_hex).context("Invalid asset hash")?;
    let policy_hash = parse_hex_32(policy_hex).context("Invalid policy hash")?;
    ...
    // Save to file
    save_json(output, &vault).context("Failed to save vault")?;
    ...
    println!(
        "  Namespace: {}",
        format!("0x{}", hex::encode(vault.namespace_id)).bright_black()
    );
    println!(
        "  Asset hash: {}",
        format!("0x{}", hex::encode(vault.asset_hash)).bright_black()
    );
    println!(
        "  Policy hash: {}",
        format!("0x{}", hex::encode(vault.policy_hash)).bright_black()
    );
    println!("  Index: {}", index);
...
pub fn verify(file: &str, namespace_file: &str) -> Result<()> {
    ...
    // Load vault
    let vault: VaultDescriptor = load_json(file).context("Failed to load vault")?;
