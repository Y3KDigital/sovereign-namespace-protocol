...
pub fn generate(seed: &str, pubkey_file: &str, seckey_file: &str) -> Result<()> {
    ...
    let (pubkey, seckey) =
        Dilithium5::keypair(seed.as_bytes()).context("Failed to generate keypair")?;

    // Save keys
    save_json(pubkey_file, &pubkey).context("Failed to save public key")?;

    save_json(seckey_file, &seckey).context("Failed to save secret key")?;
